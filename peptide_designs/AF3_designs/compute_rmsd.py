...
import numpy as np
import pandas as pd
from Bio.PDB import PDBParser, is_aa, Superimposer, MMCIFParser
...
def align_chains(pdb_file1, chain_id1, resnums1, pdb_file2, chain_id2, resnums2):
    ...
    if pdb_file1.endswith('.cif'):
        structure1 = cifparser.get_structure("struct1", pdb_file1)
    else:
        structure1 = pdbparser.get_structure("struct1", pdb_file1)

    if pdb_file2.endswith('.cif'):
        structure2 = cifparser.get_structure("struct2", pdb_file2)
    else:
        structure2 = pdbparser.get_structure("struct2", pdb_file2)
...
def align_sequences_with_gap_penalty(seq1, seq2, mode='global', match=1, mismatch=-0.1, gap_open=-100, gap_extend=-100):
    """
...
def extract_chain_residue_info(pdb_file):
    ...
    if pdb_file.endswith('.cif'):
        structure = cifparser.get_structure("struct1", pdb_file)
    else:
        structure = pdbparser.get_structure("struct1", pdb_file)
...
def align_on_mhc_and_compute_rmsds(pdbfile_1, mhc_chain_1, pep_chain_1, tcra_chain_1, tcrb_chain_1,
    ...
    tcr_rmsd = compute_rmsd(tcr_atoms_1, tcr_atoms_2)
    print(f'RMSD of TCRs aligning MHCs: {tcr_rmsd:.2f}')
    # print()
    ...
    full_rmsd = super_imposer.rms
    print(f"RMSD of full alignment: {full_rmsd:.3f}")
    print()
    print()

    return mhc_rmsd, pep_rmsd, tcr_rmsd, full_rmsd
...
df = {'peptide': [], 'mhc_rmsd': [], 'pep_rmsd': [], 'tcr_rmsd': [], 'full_rmsd': []}

for folder in os.listdir('nyeso'):
    if folder.startswith('fold') and not folder.endswith('.zip'):
        pdbfile_2 = os.path.abspath(f'nyeso/{folder}/{folder}_model_0.cif')
        peptide_sequence = folder.split('_')[-1].upper()
        mhc_rmsd, pep_rmsd, tcr_rmsd, full_rmsd = align_on_mhc_and_compute_rmsds(pdbfile_1, mhc_chain_1, pep_chain_1, tcra_chain_1, tcrb_chain_1,
                                                                                    pdbfile_2, mhc_chain_2, pep_chain_2, tcra_chain_2, tcrb_chain_2)

        df['peptide'].append(peptide_sequence)
        df['mhc_rmsd'].append(mhc_rmsd)
        df['pep_rmsd'].append(pep_rmsd)
        df['tcr_rmsd'].append(tcr_rmsd)
        df['full_rmsd'].append(full_rmsd)

df = pd.DataFrame(df)
df.to_csv('nyeso_rmsds.csv', index=False)
print(df)
print()

print()
print('nyeso v9 designs')
...
df = pd.DataFrame(df)
df.to_csv('nyeso_v9_rmsds.csv', index=False)
print(df)
print()


df_1 = pd.read_csv('nyeso_rmsds.csv')
df_2 = pd.read_csv('nyeso_v9_rmsds.csv')
df_mean = pd.merge(df_1, df_2, on='peptide', suffixes=('_v8', '_v9'))
df_mean['mhc_rmsd'] = (df_mean['mhc_rmsd_v8'] + df_mean['mhc_rmsd_v9']) / 2
df_mean['pep_rmsd'] = (df_mean['pep_rmsd_v8'] + df_mean['pep_rmsd_v9']) / 2
df_mean['tcr_rmsd'] = (df_mean['tcr_rmsd_v8'] + df_mean['tcr_rmsd_v9']) / 2
df_mean['full_rmsd'] = (df_mean['full_rmsd_v8'] + df_mean['full_rmsd_v9']) / 2
df_mean = df_mean[['peptide', 'mhc_rmsd', 'pep_rmsd', 'tcr_rmsd', 'full_rmsd']]
df_mean.to_csv('nyeso_mean_rmsds.csv', index=False)
...
pdbfile_1 = '/Users/gianmarcovisani/Desktop/tcr_pmhc/tcr_antigen_design/peptide_designs/ebv/hpvg/pdbs/3mv7.pdb.human.MH1.B-35.A.C.DE.pdb'
mhc_chain_1 = 'A'
pep_chain_1 = 'B'
tcra_chain_1 = 'C'
tcrb_chain_1 = 'D'
...
for folder in os.listdir('ebv'):
    if folder.startswith('fold') and not folder.endswith('.zip'):
        pdbfile_2 = os.path.abspath(f'ebv/{folder}/{folder}_model_0.cif')
        peptide_sequence = folder.split('_')[-1].upper()
        mhc_rmsd, pep_rmsd, tcr_rmsd, full_rmsd = align_on_mhc_and_compute_rmsds(pdbfile_1, mhc_chain_1, pep_chain_1, tcra_chain_1, tcrb_chain_1,
                                                                                    pdbfile_2, mhc_chain_2, pep_chain_2, tcra_chain_2, tcrb_chain_2)
...
df = pd.DataFrame(df)
df.to_csv('ebv_rmsds.csv', index=False)
print(df)
print()

print()
print('ebv q5 designs')

pdbfile_1 = '/Users/gianmarcovisani/Desktop/tcr_pmhc/tcr_antigen_design/peptide_designs/ebv/hpvg_q5/pdbs/4prp.pdb.human.MH1.B-35.A.C.DE.pdb'
mhc_chain_1 = 'A'
pep_chain_1 = 'B'
tcra_chain_1 = 'C'
tcrb_chain_1 = 'D'
...
df = pd.DataFrame(df)
df.to_csv('ebv_q5_rmsds.csv', index=False)
print(df)
print()

df_1 = pd.read_csv('ebv_rmsds.csv')
df_2 = pd.read_csv('ebv_q5_rmsds.csv')
df_mean = pd.merge(df_1, df_2, on='peptide', suffixes=('_v8', '_v9'))
df_mean['mhc_rmsd'] = (df_mean['mhc_rmsd_v8'] + df_mean['mhc_rmsd_v9']) / 2
df_mean['pep_rmsd'] = (df_mean['pep_rmsd_v8'] + df_mean['pep_rmsd_v9']) / 2
df_mean['tcr_rmsd'] = (df_mean['tcr_rmsd_v8'] + df_mean['tcr_rmsd_v9']) / 2
df_mean['full_rmsd'] = (df_mean['full_rmsd_v8'] + df_mean['full_rmsd_v9']) / 2
df_mean = df_mean[['peptide', 'mhc_rmsd', 'pep_rmsd', 'tcr_rmsd', 'full_rmsd']]
df_mean.to_csv('ebv_mean_rmsds.csv', index=False)

print()
print('magea3 designs')

pdbfile_1 = '/Users/gianmarcovisani/Desktop/tcr_pmhc/tcr_antigen_design/peptide_designs/magea3_and_titin/magea3/pdbs/5brz.pdb.human.MH1.A-01.A.C.DE.pdb'
mhc_chain_1 = 'A'
pep_chain_1 = 'B'
tcra_chain_1 = 'C'
tcrb_chain_1 = 'D'
...
for folder in os.listdir('magea3'):
    if folder.startswith('fold') and not folder.endswith('.zip'):
        pdbfile_2 = os.path.abspath(f'magea3/{folder}/{folder}_model_0.cif')
        peptide_sequence = folder.split('_')[-1].upper()
        mhc_rmsd, pep_rmsd, tcr_rmsd, full_rmsd = align_on_mhc_and_compute_rmsds(pdbfile_1, mhc_chain_1, pep_chain_1, tcra_chain_1, tcrb_chain_1,
                                                                                    pdbfile_2, mhc_chain_2, pep_chain_2, tcra_chain_2, tcrb_chain_2)
...
df = pd.DataFrame(df)
df.to_csv('magea3_rmsds.csv', index=False)
print(df)
print()
...
pdbfile_1 = '/Users/gianmarcovisani/Desktop/tcr_pmhc/tcr_antigen_design/peptide_designs/magea3_and_titin/titin/pdbs/5bs0.pdb.human.MH1.A-01.A.C.DE.pdb'
mhc_chain_1 = 'A'
pep_chain_1 = 'B'
tcra_chain_1 = 'C'
tcrb_chain_1 = 'D'
...
df = pd.DataFrame(df)
df.to_csv('titin_rmsds.csv', index=False)
print(df)
print()

df_1 = pd.read_csv('magea3_rmsds.csv')
df_2 = pd.read_csv('titin_rmsds.csv')
df_mean = pd.merge(df_1, df_2, on='peptide', suffixes=('_magea3', '_titin'))
df_mean['mhc_rmsd'] = (df_mean['mhc_rmsd_magea3'] + df_mean['mhc_rmsd_titin']) / 2
df_mean['pep_rmsd'] = (df_mean['pep_rmsd_magea3'] + df_mean['pep_rmsd_titin']) / 2
df_mean['tcr_rmsd'] = (df_mean['tcr_rmsd_magea3'] + df_mean['tcr_rmsd_titin']) / 2
df_mean['full_rmsd'] = (df_mean['full_rmsd_magea3'] + df_mean['full_rmsd_titin']) / 2
df_mean = df_mean[['peptide', 'mhc_rmsd', 'pep_rmsd', 'tcr_rmsd', 'full_rmsd']]
df_mean.to_csv('magea3_titin_mean_rmsds.csv', index=False)

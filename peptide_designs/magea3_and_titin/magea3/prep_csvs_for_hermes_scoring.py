...
pdb = '5brz.pdb.human.MH1.A-01.A.C.DE'
chain = 'B'
...
# df['pdb_for_hermes_scoring'] = [pdb for _ in range(len(df))]
# df['chain_for_hermes_scoring'] = [chain for _ in range(len(df))]
# df.to_csv('hcnn_pyrosetta_annealing/hcnn_plus_pyrosetta_annealing_peptides_so3_convnet_noise=0p5_w_pae_w_blosum.tsv', index=None, sep='\t')


## wildtype
df = pd.read_csv('wildtype/wildtype_w_pae_w_blosum.tsv', sep='\t')
df['pdb_for_hermes_scoring'] = [pdb for _ in range(len(df))]
df['chain_for_hermes_scoring'] = [chain for _ in range(len(df))]
df.to_csv('wildtype/wildtype_w_pae_w_blosum.tsv', index=None, sep='\t')

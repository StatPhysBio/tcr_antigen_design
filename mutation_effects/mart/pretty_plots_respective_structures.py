...
if __name__ == '__main__':
    ...
    system = args.system
    assert '_{pdb}' in args.system_name_in_csv_file
    ...
    if model == 'hermes':
        ...
        if 'relaxation' in model_instance:
            df_full = pd.read_csv(os.path.join(base_dir, f'{system_name_in_csv_file.format(pdb="averaged")}-{model_instance}-use_mt_structure={use_mt_structure}.csv'))
        else:
        ...
        title = f'{system_name_in_csv_file.format(pdb="resp_struc")}\n{model} - {model_instance}\nuse_mt_structure={use_mt_structure}'
        ...
        out_file = f'pretty_scatterplot-{system_name_in_csv_file.format(pdb="resp_struc")}-{model_instance}-num_seq_per_target={num_seq_per_target}-use_mt_structure={use_mt_structure}.png'
    ...
    predictions = df_full[prediction_column]
    targets = df_full[target_column]

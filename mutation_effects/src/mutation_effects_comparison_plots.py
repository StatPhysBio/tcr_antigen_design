...
def get_long_prediction_column_name(model_instance, prediction_column_short, system_name_in_csv_file):
    ...
    elif 'blosum62' in model_instance or 'luksza_cross_reactivity' in model_instance:
        if prediction_column_short == 'sub_score':
            return 'substitution_matrix_score', system_name_in_csv_file
        else:
            raise ValueError(f'Unknown prediction_column_short: {prediction_column_short}')
    
    elif model_instance == 'tulip':
        if prediction_column_short == 'TULIP_SCORE':
            return 'TULIP_SCORE', system_name_in_csv_file
        else:
            raise ValueError(f'Unknown prediction_column_short: {prediction_column_short}')
    ...
    elif model_instance == 'nettcr2p2':
        if prediction_column_short == 'nettcr_score':
            return 'nettcr_score', system_name_in_csv_file
        else:
            raise ValueError(f'Unknown prediction_column_short: {prediction_column_short}')
...
def make_pretty_name(model, pred_col):
    if model == 'hermes_py_000' and pred_col in {'delta_log_p', 'pE-fixed'}:
        ...
        return 'BLOSUM62'
    elif model == 'luksza_cross_reactivity' and pred_col == 'sub_score':
        return 'Luksza et al. $C$'
    elif model == 'luksza_cross_reactivity_without_d' and pred_col == 'sub_score':
        return 'Luksza et al. $C/d$'
    elif model == 'tulip' and pred_col == 'TULIP_SCORE':
        return 'TULIP'
    elif model == 'tapir' and pred_col == 'tapir_score':
        return 'TAPIR'
    elif model == 'nettcr2p2' and pred_col == 'nettcr_score':
        return 'NetTCR-2.2'
    else:
...
def get_model_specific_parameters(model_instance, prediction_column_short, args):
    ...
    elif 'blosum62' in model_instance or 'luksza_cross_reactivity' in model_instance or 'luksza_cross_reactivity_without_d' in model_instance:
        # print('Note: "use_mt_structure" is irrelevant with neg_abs_diff_vdw_radius substitution matrix.', file=sys.stderr)
        base_dir = f'{MUT_EFFECTS_DIR}/{system}/results/{model_instance}/'
...
if __name__ == '__main__':
    ...
    os.makedirs(f'{MUT_EFFECTS_DIR}/{args.system}/plots', exist_ok=True)
    plt.tight_layout()
    plt.savefig(f'{MUT_EFFECTS_DIR}/{args.system}/plots/{args.system_name_in_csv_file}_scatterplots.png')
    plt.savefig(f'{MUT_EFFECTS_DIR}/{args.system}/plots/{args.system_name_in_csv_file}_scatterplots.pdf')
    plt.close()
    ...
    os.makedirs(f'{MUT_EFFECTS_DIR}/{args.system}/plots', exist_ok=True)
    plt.tight_layout()
    plt.savefig(f'{MUT_EFFECTS_DIR}/{args.system}/plots/{args.system_name_in_csv_file}_barplots.png')
    plt.savefig(f'{MUT_EFFECTS_DIR}/{args.system}/plots/{args.system_name_in_csv_file}_barplots.pdf')
    plt.close()
    ...
    # save the data
    with open(f'{MUT_EFFECTS_DIR}/{args.system}/results/{args.system_name_in_csv_file}_metrics.json', 'w') as f:

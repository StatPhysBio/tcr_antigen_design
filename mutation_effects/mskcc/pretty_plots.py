...
CUTOFF_BAD_VALUE = {
    'mskcc_tcr1_ec50_sat_mut': -6,
    'mskcc_tcr2_ec50_sat_mut': -6,
    'mskcc_tcr3_ec50_sat_mut': -6,
    'mskcc_tcr4_ec50_sat_mut': -8,
    'mskcc_tcr5_ec50_sat_mut': -8,
    ...
    'mskcc_tcr4_ec50_sat_mut_with_relaxation': -8,
...
if __name__ == '__main__':
    ...
    if model == 'hermes':
        ...
        print('Note: "use_mt_structure" is irrelevant with static substitution matrix.', file=sys.stderr)
        base_dir = os.path.join(args.base_dir, f'{system}/results/{model_instance}/')
    ...
    if system == 'mskcc_tcr4_ec50_sat_mut':
        # merge with PCA_parametrization
        df_pca = pd.read_csv('PCA_parametrization.csv')
        renaming_dict = {'mutant': 'Peptide'}
        df_full = df_full.rename(columns=renaming_dict)
        df_pca['Peptide'] = [f'{elem[0]}{int(elem[1:-1])+375}{elem[-1]}' for elem in df_pca['Peptide']]
        df_full = pd.merge(df_full, df_pca, on='Peptide')

    out_file = out_file.replace('.png', f'{args.target_column}.png')

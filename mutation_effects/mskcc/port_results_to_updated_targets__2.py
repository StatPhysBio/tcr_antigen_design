...
columns_to_drop = ['-log10(EC50)', 'is_reliable']
columns_to_drop_backup = []
columns_to_merge_over = ['mutant', 'sequence', 'wt_seq', 'wt_pdb', 'mt_pdb', 'is_wt', 'mutant_chain']
columns_to_merge_over_backup = ['mutant', 'sequence', 'wt_pdb', 'mt_pdb', 'is_wt', 'mutant_chain']
...
for model in models:
    ...
    for tcr in tcrs:
        ...
        for relaxation_str in ['', '_with_relaxation']:
            ...
            try:
                df_old_pred = pd.read_csv(f'results_old2/{model}/{identifier}{relaxation_str}-{model}-use_mt_structure=0.csv')
...
for model in models:
    ...
    for tcr in tcrs:
        ...
        try:
            df_old_pred = pd.read_csv(f'results_old2/{model}/zero_shot_predictions/{identifier}-num_seq_per_target=10-use_mt_structure=0.csv')
        except FileNotFoundError:
            continue
...
for model in models:
    ...
    for tcr in tcrs:
        ...
        try:
            df_old_pred = pd.read_csv(f'results_old2/{model}/{identifier}-{model}-use_mt_structure=0.csv')
        except FileNotFoundError:
            continue
...
columns_to_drop = ['-log10(EC50)', 'is_reliable', 'mutant', 'mutant_chain'] # mutant and mutant_chain were with tcrdock numbering so wrong
columns_to_merge_over = ['peptide', 'is_wt', 'wt_pdb', 'mt_pdb', 'wt_seq']
...
for model in models:
    ...
    for tcr in tcrs:
        ...
        try:
            if model == 'tcrdock':
                df_old_pred = pd.read_csv(f'results_old2/{model}/{identifier}_w_pae.tsv', sep='\t')
            else:
                df_old_pred = pd.read_csv(f'results_old2/{model}/{identifier}_no_nearby_templates_w_pae.tsv', sep='\t')
        except FileNotFoundError:
            continue
        ...
        os.makedirs(f'results/{model}', exist_ok=True)
        if model == 'tcrdock':
            df_merged.to_csv(f'results/{model}/{identifier}_w_pae.tsv', sep='\t', index=False)
        else:
